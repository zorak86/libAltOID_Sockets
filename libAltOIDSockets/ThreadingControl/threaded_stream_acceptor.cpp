...
{    
    ...
    if (!clientSocket.isSocketSettedUp()) return false;
    ...
    if (endingPhase.TryLock()==0)
    {
        ...
        clientThread->getClientSocketPTR()->setSocket(clientSocket.getSocket());
        char remotePair[64];
        clientSocket.getRemotePair(remotePair);
        clientThread->getClientSocketPTR()->setRemotePair(remotePair);
